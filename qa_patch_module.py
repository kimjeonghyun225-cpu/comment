...
def build_system_prompt() -> str:
    return (
        ...
        "조사형 표현 위주, 중복·장황 서술 지양. 출력은 반드시 JSON입니다.\n"
        "\n"
        "반드시 지킬 규칙:\n"
        "1) 출력은 JSON 하나뿐이며 아래 스키마를 엄격 준수합니다.\n"
        "2) GPU/Chipset 군집 요약 이슈를 issues[] 안에 최소 1건 이상 포함합니다.\n"
        "   - 근거: metrics.clusters.by_gpu / by_chipset 상위 분포 + metrics.by_issue_tag 상위 태그\n"
        "   - 군집이 유의미하지 않으면 생략 가능하나, 유의미한 경우 반드시 포함\n"
        "3) 각 군집 요약 이슈는 다음 형태를 따릅니다.\n"
        "   - title: \"[CLUSTER][GPU] {GPU명}\" 또는 \"[CLUSTER][Chipset] {칩셋명}\"\n"
        "   - symptom: 대표 증상(상위 태그 기반, 예: 해상도/회전/렌더링/성능 등)\n"
        "   - reproduction: 재현 조건(예: 화면 회전/해상도 변경/장시간 플레이 등)\n"
        "   - evidence: sample_issues에서 해당 군집의 대표 코멘트 일부(최대 3개)\n"
        "   - impact: 사용자 영향(시나리오 중심)\n"
        "   - priority: P0/P1/P2 (분포·심각도 기준 추정)\n"
        "   - cause: GPU/칩셋 특이 처리/최적화 미흡 등 데이터로 정당화 가능한 범위만 기술\n"
        "   - recommendation: 해당 군집 대상 보정/회귀 테스트/가이드 반영\n"
        "\n"
        + JSON_SCHEMA_DOC
    )
...
def build_user_prompt(
    ...
    guideline = (
        ...
        "[지시]\n"
        "1) 한 줄 총평(≤300자)\n"
        "2) issues[] 작성 (개별 핵심 이슈 + 군집 요약 이슈 포함)\n"
        "   2-1) 개별 핵심 이슈: 증상/재현/근거/영향/우선순위/원인/권고\n"
        "   2-2) GPU/Chipset 군집 요약 이슈(있을 경우 필수):\n"
        "       - title: \"[CLUSTER][GPU] {상위 GPU명}\" 또는 \"[CLUSTER][Chipset] {상위 칩셋명}\"\n"
        "       - symptom: metrics.by_issue_tag 상위 태그를 근거로 대표 증상 1~2개 요약\n"
        "       - reproduction: 해당 증상이 잘 드러나는 조건(회전/해상도 변경/특정 화면/장시간 등)\n"
        "       - evidence: sample_issues 내 해당 군집의 코멘트에서 2~3개 발췌(요약)\n"
        "       - impact/priority/cause/recommendation: 데이터 범위 내에서 간결히\n"
        "   2-3) 군집이 유의미하지 않으면 생략\n"
        "3) device_risks[]: (기존과 동일)\n"
        "4) actions[]: (기존과 동일)\n"
        "5) release_decision + conditions\n"
        "[근거 데이터]\n"
        "- 군집 분포: metrics.clusters.by_gpu / by_chipset\n"
        "- 전역 증상 분포: metrics.by_issue_tag\n"
        "- 코멘트 원문: sample_issues.comment_text\n"
        "6) GPU/Chipset 군집 요약: metrics.clusters.by_gpu / by_chipset 과 공통 이슈내용을 활용하여\n"
        ...
        "7) GPU/Chipset 군집 요약을 Summary에도 반드시 반영하라\n"
        "8) 공통 Feature 군집(펀치홀/노치/회전/입력지연/렌더 아티팩트 등) 요약: metrics.clusters_feature_detailed 활용\n"
        "9) 핵심 이슈(증상/재현/근거/영향/우선순위/원인·권고)\n"
        "10) 디바이스 리스크(조합·원인 추정)\n"
        "11) 액션플랜(담당/기한/검증기준)\n"
        "12) 릴리스 권고(Go/No-Go/Conditional+조건)\n"
